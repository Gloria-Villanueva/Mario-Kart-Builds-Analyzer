...
) : drivers(drivers), bodies(bodies), tires(tires), gliders(gliders) {}
...
vector<Build> QueryEngine::ranking(int topN) const {
    ...
    for (const Build& build : todos) {
        ...
        for (const Build& u : unicos) {
            if (abs(build.getPerformanceScore() - u.getPerformanceScore()) < 0.001) {
                yaVisto = true; break;
            }
        }
...
vector<Build> QueryEngine::similares(const Build& referencia, int k) const {
...
double QueryEngine::agregacion(
    const string& tipoFiltro, const string& valorFiltro, const string& operacion) const {
    ...
    if (scores.empty())
        throw MK8Exception(
            "No se encontraron componentes con " + tipoFiltro + " = " + valorFiltro);
